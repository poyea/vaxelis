...
void Input::bind_action(std::string name, std::initializer_list<SDL_Scancode> keys) {
    ...
    for (auto k : keys) b.keys.push_back(k);
}

void Input::clear_actions() { actions_.clear(); }
...
bool Input::down(std::string_view name) const {
    ...
    for (auto k : it->second.keys) if (down(k)) return true;
    return false;
}
bool Input::pressed(std::string_view name) const {
    auto it = actions_.find(std::string(name));
    if (it == actions_.end()) return false;
    for (auto k : it->second.keys) if (pressed(k)) return true;
    return false;
}
bool Input::released(std::string_view name) const {
    auto it = actions_.find(std::string(name));
    if (it == actions_.end()) return false;
    for (auto k : it->second.keys) if (released(k)) return true;
    return false;
}
