...
public:
    ...
    void draw_textured_quad(ShaderHandle sh, BufferHandle vb, BufferHandle ib,
                            uint32_t index_count, TextureHandle tex,
                            const mat4& mvp) override {
        ...
        if (s_it == shaders_.end() || v_it == buffers_.end() ||
            ...
            VX_ERROR("draw_textured_quad: invalid handle");
            return;
        }
        gl().UseProgram(s_it->second.program);
        gl().UniformMatrix4fv(s_it->second.mvp_loc, 1, GL_FALSE, &mvp[0][0]);
        ...
        // pos(vec2) + uv(vec2), interleaved, stride = 16
        gl().VertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 16, reinterpret_cast<void*>(0));
        gl().EnableVertexAttribArray(0);
        gl().VertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 16, reinterpret_cast<void*>(8));
