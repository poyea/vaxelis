...
bool aabb_overlap(vaxelis::vec2 amin, vaxelis::vec2 amax,
                  vaxelis::vec2 bmin, vaxelis::vec2 bmax) {
    return amin.x < bmax.x && amax.x > bmin.x && amin.y < bmax.y && amax.y > bmin.y;
}

struct AABB { vaxelis::vec2 min, max; };
...
const char* level_path(int idx) {
    switch (idx) {
        case 1: return "assets/maps/level1.tmj";
        case 2: return "assets/maps/level2.tmj";
        case 3: return "assets/maps/level3.tmj";
    }
...
protected:
    ...
    void on_fixed_update(float dt) override {
        ...
        if (input().pressed("restart")) { load_level(current_level_); return; }
...
private:
    ...
    void drive_player(float dt) {
        ...
        float vx = 0.0f;
        if (input().down("move_left"))  vx -= 240.0f / ppm;
        if (input().down("move_right")) vx += 240.0f / ppm;
    ...
    void update_enemies(float dt) {
        ...
        for (auto e : enemies_) {
            ...
            if (tr.position.x > en.origin_x + en.range) { tr.position.x = en.origin_x + en.range; en.dir = -1; }
            if (tr.position.x < en.origin_x - en.range) { tr.position.x = en.origin_x - en.range; en.dir =  1; }
        }
    }
    ...
    void check_triggers() {
        ...
        for (const auto& h : hazards_) {
            if (aabb_overlap(pl.min, pl.max, h.min, h.max)) { kill_player(); return; }
        }

        // Enemies: from-above contact squashes; otherwise dies.
        auto& reg = scene_.registry();
        const auto& player_rb = reg.get<vaxelis::RigidBody2D>(player_);
        const b2Vec2 pv = B2_IS_NULL(player_rb.body) ? b2Vec2{0,0} : b2Body_GetLinearVelocity(player_rb.body);
        for (auto e : enemies_) {
            if (!reg.valid(e)) continue;
            auto& en = reg.get<Enemy>(e);
            if (!en.alive) continue;
            const auto eb = sprite_aabb(e);
            if (!aabb_overlap(pl.min, pl.max, eb.min, eb.max)) continue;
